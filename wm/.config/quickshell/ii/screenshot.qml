...
ShellRoot {
    ...
    property string saveDir: Directories.screenshotDir
    ...
    Variants {
	    ...
	    property string finalImageName: ""
	    ...
	    return `Screenshot_${year}-${month}-${day}_${hours}-${minutes}-${seconds}.png`;
		    ...
		    panelWindow.finalImageName = panelWindow.generateFilename();
		    panelWindow.finalImagePath = `${root.saveDir}/${panelWindow.finalImageName}`
                    snipProc.startDetached();
                    Qt.quit();
                }
                command: ["bash", "-c", 
		    `mkdir -p ${root.saveDir} &&`
                    + `magick ${StringUtils.shellSingleQuoteEscape(panelWindow.screenshotPath)} `
                    + `-crop ${panelWindow.regionWidth * panelWindow.monitorScale}x${panelWindow.regionHeight * panelWindow.monitorScale}+${panelWindow.regionX * panelWindow.monitorScale}+${panelWindow.regionY * panelWindow.monitorScale} ${StringUtils.shellSingleQuoteEscape(panelWindow.finalImagePath)} &&` 
                    + `wl-copy < '${StringUtils.shellSingleQuoteEscape(panelWindow.finalImagePath)}' && `
        	    + `${panelWindow.mouseButton === Qt.RightButton ? `swappy -f '${StringUtils.shellSingleQuoteEscape(panelWindow.finalImagePath)}' && ` : ""}`
		    + "notify-send --icon=photo "
		    + `"Screenshot saved!" `
		    + `"${panelWindow.finalImageName} saved in ${root.saveDir}" `
		    + `-a "Shell" `
	    ]
            }
